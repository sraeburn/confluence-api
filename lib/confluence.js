...
/**
 * Post content + attachments to a new page
 *
 * @param {string} space
 * @param {string} title
 * @param {string} content
 * @param {number} parentId - A null value will cause the page to be added under the space's home page
 * @param {object|array} attachments - One or more page attachments
 * @param {Function} callback
 */
Confluence.prototype.postContentWithAttachments = function(space, title, content, parentId, attachments, callback){
    self = this;
    self.postContent(space, title, content, parentId, function(err, data) {
       if (err) callback(err);
       let newPageId = data.id;
        if (attachments) {
            ...
            attachments.forEach(attachment => {
                ...
                self.createAttachment(space, newPageId, attachment.file, attachment.options, function (err, data) {
                    if (err) callback(err);
                });
            });
        }
 ...
 * @param {string} space
 * @param {string} id
 * @param {String|fs.ReadStream|Buffer} file — either string with file path or Blob/Buffer object.
 * @param {Function} callback
 */
// Confluence.prototype.createAttachment = function(space, id, file, callback ){
 ...
 *
 * @param {String|fs.ReadStream|Buffer} file — either string with file path or Blob/Buffer object.
 * @param {String|Object} options  - string with custom filename or options object \
 e.g. {filename: string, contentType: 'mime/type'}
 * @param {Function} callback
 */
Confluence.prototype.createAttachment = function(space, id, file, options, callback ){
//https://docs.atlassian.com/atlassian-confluence/REST/latest/#content/{id}/child/attachment-getAttachments
	var args = [...arguments];
	callback = args.pop();
	if (args.length < 4) {
